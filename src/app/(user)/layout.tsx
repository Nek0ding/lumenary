...
import {
    ...
    Settings, LogOut, Search, Bell, BookOpen
} from 'lucide-react';
...
export default function UserLayout({ children }: { children: React.ReactNode }) {
    ...
    // --- STATE PENCARIAN (Global Dashboard Navbar) ---
    ...
    const handleSearchSubmit = (e: React.FormEvent) => {
        e.preventDefault(); // Mencegah form me-refresh halaman sepenuhnya
        if (searchQuery.trim() !== '') {
            // Arahkan ke halaman explore dengan query di URL
            router.push(`/explore?q=${encodeURIComponent(searchQuery)}`);
            // Opsional: Kosongkan kotak pencarian setelah dikirim agar bersih
            setSearchQuery(''); 
        } else {
            router.push(`/explore`);
        }
    ...
    return (
        <div className="flex h-screen w-full bg-[#F8F8FF] font-['Plus_Jakarta_Sans',sans-serif]">
            ...
            <aside className="w-[260px] bg-white border-r border-zinc-200 flex flex-col justify-between hidden md:flex">
                <div>
                    {/* Logo */}
                    <div className="flex items-center gap-3 px-8 py-8 cursor-pointer" onClick={() => router.push('/')}>
