  ...
  return (
    <div className="w-full bg-white rounded-[16px] md:rounded-[20px] border-b-[4px] border-zinc-300 shadow-[0px_4px_8px_rgba(0,0,0,0.06)] hover:border-zinc-400 hover:shadow-[0px_6px_12px_rgba(0,0,0,0.08)] transition-all duration-300 overflow-hidden">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full px-5 py-4 md:px-8 md:py-5 flex justify-between items-center cursor-pointer focus:outline-none">
        <span className="text-[#020617] text-[15px] sm:text-[16px] md:text-[20px] font-bold tracking-tight text-left">{question}</span>
      ...
      <div className={`px-5 md:px-8 transition-all duration-300 ease-in-out ${isOpen ? 'max-h-40 pb-4 md:pb-5 opacity-100' : 'max-h-0 pb-0 opacity-0'}`}>
        <p className="text-[13px] sm:text-[14px] md:text-[18px] font-medium leading-relaxed">{answer}</p>
      </div>
    </div>
  );
};
  ...
  return (
    <>
      ...
      {isModalOpen && selectedBook && (
        ...
        >
          <div
            className="flex flex-col md:flex-row relative no-scrollbar p-6 md:p-8 gap-6 md:gap-8 animate-in zoom-in-95 duration-200"
            style={{
          ...
          >
            ...
            <div className="flex flex-col items-center w-full max-w-[160px] sm:max-w-[200px] md:max-w-[280px] shrink-0 mx-auto">
              <img
            ...
            <div className="flex flex-col flex-1 pt-0 md:pt-2">
              <h2 className="text-[20px] sm:text-[24px] md:text-[32px] font-extrabold text-[#111] mb-1 md:mb-2 leading-[1.2]">
                {selectedBook.judul}
              </h2>
              <p className="text-[14px] md:text-[20px] font-medium text-[#555] mb-3 md:mb-4">
                {selectedBook.penulis}
              </p>

              <div className="flex flex-wrap items-center gap-2 mb-3 md:mb-4">
                <div style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: selectedBook.stok > 0 ? '#00A651' : '#E11D48' }} />
                <span className="text-[12px] md:text-[15px] font-bold" style={{ color: selectedBook.stok > 0 ? '#00A651' : '#E11D48' }}>
                  {selectedBook.stok > 0 ? 'Available for Loan' : 'Out of Stock'}
                </span>
                <span className="text-[#A1A1AA] mx-1 hidden sm:block">|</span>
                <span className="text-[12px] md:text-[15px] text-[#111] font-medium w-full sm:w-auto">
                  Stock leftovers: <b className="font-extrabold">{selectedBook.stok > 0 ? `${selectedBook.stok} Books` : '0 Books'}</b>
                </span>
              </div>

              <div className="flex flex-wrap gap-2 md:gap-3 mb-4 md:mb-6">
                <span className="bg-[#EADFFF] text-[#6B21A8] px-3 py-1 md:px-4 md:py-[6px] rounded-lg text-[11px] md:text-[14px] font-bold">
                  {selectedBook.kategori?.nama_kategori || 'Sci-Fi'}
                </span>
              </div>

              <h3 className="text-[16px] md:text-[20px] font-extrabold text-[#111] mb-2 md:mb-3">Synopsis</h3>
              <p className="text-[13px] md:text-[16px] text-[#222] leading-[1.6] mb-5 md:mb-6 font-medium whitespace-pre-wrap">
                {renderSynopsis()}
              </p>
              ...
              <div className="mt-auto flex flex-col gap-3">
                <button
                  ...
                  className="w-full py-3 md:py-4 rounded-xl text-[15px] md:text-[18px] font-extrabold border-none text-white shadow-md active:scale-95 transition-transform"
                  style={{
                ...
                <p className="text-center text-[11px] md:text-[14px] text-[#555] mt-1 font-medium">
      ...
      <div className="w-full bg-white min-h-screen pt-4 pb-10 px-4 sm:px-6 md:px-[80px] font-['Plus_Jakarta_Sans',sans-serif] text-[#020617] antialiased flex justify-center relative no-scrollbar">
        <div className={`w-full max-w-[1200px] flex flex-col gap-14 md:gap-[40px] ${isModalOpen ? 'blur-sm pointer-events-none' : ''} transition-all duration-300`}>

          {/* HERO BOX */}
          <div
            className="relative w-full min-h-[500px] md:min-h-[680px] rounded-[32px] md:rounded-[40px] overflow-hidden shadow-sm flex flex-col justify-between p-6 sm:p-8 md:p-[40px]"
            style={{ background: 'linear-gradient(102.43deg, #FFFEF7 0%, #E2C6FF 83.81%)' }}
          >
            <div className="absolute pointer-events-none hidden md:block rounded-br-[40px]" style={{ width: '988px', height: '415px', left: '271px', bottom: '0px', background: 'linear-gradient(107.93deg, #FFDFEC 22.78%, #B8BAE3 38.05%, #555CDA 61.53%)', opacity: 0.9, filter: 'blur(50px)', zIndex: 0 }} />
            ...
            <div className="relative z-10 w-full flex flex-col justify-between flex-1 gap-10">
              ...
              <div className="w-full flex items-center justify-between gap-4 cursor-pointer">
                <div className="flex items-center gap-2 md:gap-3" onClick={landingPage}>
                  <img className="w-[36px] h-[36px] sm:w-[40px] sm:h-[40px] md:w-[50px] md:h-[50px] object-contain" src="/logo.png" alt="Logo" />
                  <div className="flex flex-col gap-[2px]">
                    <span className="text-[#161B85] text-[18px] sm:text-[20px] md:text-[24px] font-bold tracking-wide leading-none">Lumenary</span>
                    <span className="text-[#492073] text-[6px] sm:text-[7px] md:text-[8px] font-bold uppercase tracking-wider">Gunadarma Library</span>
                  </div>
                </div>

                <div className="hidden lg:flex items-center gap-12">
                  <a onClick={landingPage} className="text-[#020617] text-[18px] xl:text-[20px] font-bold hover:opacity-70 transition-opacity">Home</a>
                  <a onClick={explore} className="text-[#020617] text-[18px] xl:text-[20px] font-bold hover:opacity-70 transition-opacity">Explore Books</a>
                  <a href="#features" className="text-[#020617] text-[18px] xl:text-[20px] font-bold hover:opacity-70 transition-opacity">Features</a>
                  <a href="#about" className="text-[#020617] text-[18px] xl:text-[20px] font-bold hover:opacity-70 transition-opacity">About</a>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={getStarted}
                    className="hidden sm:block relative z-50 cursor-pointer px-5 py-2 md:px-6 md:py-2.5 text-[#FCFDE9] text-[14px] md:text-[18px] font-bold rounded-full shadow-[0px_2px_4px_rgba(0,0,0,0.25)] hover:brightness-110 transition-all active:scale-95 border-none"
              ...
              <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8 items-center flex-1 pb-28 sm:pb-24 md:pb-24">
                <div className="flex flex-col gap-[16px] md:gap-[24px] max-w-[540px]">
                  <div className="flex flex-col gap-[8px] md:gap-[12px]">
                    <h2 className="text-[#020617] text-[26px] sm:text-[32px] font-bold leading-[1.2] tracking-tight">Your Next Chapter Starts Here</h2>
                    <h2 className="text-[26px] sm:text-[31px] font-bold leading-tight">
                  ...
                  <Link href="/explore">
                    <button className="cursor-pointer w-fit px-6 py-[12px] md:px-8 md:py-[14px] text-[#FCFDE9] text-[16px] sm:text-[20px] md:text-[24px] font-bold rounded-full flex items-center gap-[10px] md:gap-[12px] shadow-[0px_4px_6px_rgba(0,0,0,0.15)] group hover:brightness-105 transition-all" style={{ background: 'linear-gradient(120deg, #DDDEF2 10%, #8EA1E6 45%, #3037B4 77%, #101464 100%)' }}>
                      Explore Library
              ...
              {/* SEARCH BAR */}
              <div className="absolute bottom-6 sm:bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] sm:w-[90%] max-w-[842px] h-[56px] sm:h-[60px] md:h-[72px] bg-white px-4 sm:px-5 md:px-7 py-3 md:py-4 rounded-[16px] md:rounded-[20px] shadow-[0px_8px_16px_rgba(0,0,0,0.08)] flex items-center gap-[10px] md:gap-[20px] border border-zinc-100 z-20">
                <Search className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 text-zinc-400 shrink-0" />
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  className="w-full text-zinc-700 text-[14px] sm:text-[16px] md:text-[20px] font-medium placeholder-[#B1B2B7] bg-transparent outline-none"
                />
              </div>
          ...
          {/* ================= TRENDING NOW ================= */}
          <div className="w-full flex flex-col items-center gap-[24px] md:gap-[32px] pt-10 md:pt-14 px-2 md:px-5">
            <div className="text-center flex flex-col gap-[6px] md:gap-[12px]">
              <h2 className="text-[24px] md:text-[32px] font-extrabold tracking-tight">Trending Now</h2>
                ...
                {trendingBooks.length > 0 ? (
                  trendingBooks.map((book) => (
                    <div key={book.id_buku} onClick={() => openBookDetail(book)} className="flex flex-col gap-[10px] md:gap-[12px] group cursor-pointer hover:-translate-y-2 transition-transform duration-300">
                      <div className="overflow-hidden rounded-[16px] md:rounded-[20px] shadow-sm group-hover:shadow-md transition-shadow aspect-[3/4] bg-zinc-100 relative border border-zinc-100">
                      ...
                      <div className="flex flex-col gap-[2px] md:gap-[4px]">
                        <h3 className="text-[14px] md:text-[16px] font-bold leading-tight line-clamp-1 md:truncate text-[#020617]" title={book.judul}>{book.judul}</h3>
                        <p className="text-zinc-500 text-[12px] font-normal truncate">{book.penulis}</p>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="col-span-full text-center py-6 text-[14px] md:text-[18px]">Tidak ada buku trending minggu ini.</div>
                )}
              </div>
            )}
          </div>
          ...
          <div className="w-full flex flex-col items-center gap-[24px] md:gap-[32px] pt-4 px-2 md:px-5" id="features">
            <div className="text-center flex flex-col gap-[6px] md:gap-[12px]">
              <h2 className="text-[24px] md:text-[32px] font-extrabold tracking-tight">How It Works?</h2>
              <p className="text-[14px] sm:text-[16px] md:text-[24px] font-medium text-zinc-600 md:text-[#020617]">Discover, reserve, and enjoy your favorite books in just three simple steps</p>
            </div>
            <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              {[
                ...
                <div key={idx} className="p-6 md:p-8 bg-white rounded-[20px] md:rounded-[24px] border border-zinc-100 shadow-[0px_2px_4px_rgba(0,0,0,0.06)] flex flex-col gap-[8px] md:gap-[12px] hover:shadow-[0px_15px_30px_rgba(0,0,0,0.08)] transition-shadow duration-300">
                  ...
                  <h3 className="text-[20px] sm:text-[24px] md:text-[32px] font-bold">{item.title}</h3>
                  <p className="text-[14px] sm:text-[15px] md:text-[20px] font-medium leading-relaxed text-zinc-600 md:text-[#020617]">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="w-full p-6 md:p-10 rounded-[24px] md:rounded-[40px] flex flex-col items-center gap-[24px] md:gap-[32px] mt-4" id="about" style={{ background: 'linear-gradient(0deg, #FFFFFF 15%, #E2C6FF 100%)' }}>
            <div className="text-center flex flex-col gap-[6px] md:gap-[12px]">
              <h2 className="text-[24px] md:text-[32px] font-extrabold tracking-tight">Frequently Asked Question</h2>
              <p className="max-w-[750px] text-[#020617] text-[14px] sm:text-[15px] md:text-[20px] font-medium leading-relaxed">Find quick answers to common questions about book reservations, slot booking, loan policies, and how to make the most of your Lumenary account.</p>
            </div>
