...
export default function LumenaryApp() {
    // Authentication State (Mengontrol Tampilan Hero Figma vs Dashboard)
    const [isLoggedIn, setIsLoggedIn] = useState<boolean>(false);
    // Form States untuk Hero Page
    ...
    // Form Submission Handler untuk Sign Up
    const handleSignUpSubmit = (e: React.FormEvent) => {
        ...
        // Berhasil Registrasi, ganti state untuk memunculkan dashboard bawaan
        setIsLoggedIn(true);
    ...
    return (
        <div className="min-h-screen bg-[#FEFEFE] text-black font-sans antialiased">
            ...
            {/* CONDITIONAL RENDERING: SIFAT TAMPILAN DEPENDS ON LOGGED IN STATUS */}
            {!isLoggedIn ? (

                /* IMPLEMENTASI HERO LANDING PAGE / SIGN UP DARI FIGMA */
                <div className="min-h-screen flex items-center justify-center p-4 md:p-0 bg-[#FEFEFE]">
                    {/* Wrapper Utama sesuai Frame 305 */}
                    <div className="w-full max-w-[1196px] min-h-screen md:h-[1024px] mx-auto flex flex-col md:flex-row items-center justify-between gap-[65px]">
                        ...
                        {/* SISI KIRI: Frame 304 (Hero Banner dengan Background Gambar & Info) */}
                        <div
                            ...
                            style={{
                                backgroundImage: `linear-gradient(360deg, rgba(16, 20, 100, 0.3) 0%, rgba(16, 20, 100, 0.6) 100%), url('/person-cafe-reading-book-while-having-coffee.jpg')`
                            }}
                        >
                            {/* LOGO LUMENARY (Sudah disamakan dengan Landing Page kamu) */}
                            <div className="absolute top-8 left-6 md:left-[141px] flex items-center gap-3">
                                ...
                                <div className="flex flex-col items-start gap-[2px]">
                                    <span className="text-[#161B85] text-[24px] font-bold tracking-wide leading-none font-sans">
                                        Lumenary
                                    </span>
                                    <span className="text-[#492073] text-[8px] font-bold uppercase tracking-wider">
                                        Gunadarma Library
                                    </span>
                                </div>
                            </div>

                            {/* Konten Teks Hero (Frame 306) */}
                            <div className="w-full max-w-[396px] flex flex-col gap-3 mt-16 md:mt-0">
                        ...
                        {/* SISI KANAN: Frame 80 & Frame 76 (Form Pendaftaran / Registrasi) */}
                ...
                /* SECTION MATRIKS UTAMA / DASHBOARD LAMA YANG TETAP DI-PRESERVE UTUH */
                    ...
                    <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white p-6 justify-between flex-shrink-0">
                        <div className="flex flex-col gap-8">
                            <div className="flex items-center gap-3 border-b border-slate-800 pb-5">
                                ...
                                <div>
                                    <h3 className="font-bold text-sm tracking-wide">Lumenary</h3>
                                    <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Gunadarma Library</span>
                                </div>
                            </div>
                            ...
                            <nav className="flex flex-col gap-1">
                                ...
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all relative ${activeTab === 'reservations' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'
                                        }`}
                                >
                    ...
                    <header className="md:hidden w-full bg-slate-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-40">
                        ...
                        {mobileMenuOpen && (
                            <div className="absolute top-full left-0 w-full bg-slate-900 border-t border-slate-800 p-4 shadow-xl flex flex-col gap-2">
                                <button onClick={() => { setActiveTab('browse'); setMobileMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg text-sm font-bold ${activeTab === 'browse' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`} > Browse Books </button>
                                <button onClick={() => { setActiveTab('reservations'); setMobileMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg text-sm font-bold flex justify-between items-center ${activeTab === 'reservations' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`} > My Holds {reservations.length > 0 && <span className="bg-amber-500 text-slate-950 text-xs px-2 py-0.5 rounded font-bold">{reservations.length}</span>} </button>
                                <button onClick={() => { setActiveTab('faq'); setMobileMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg text-sm font-bold ${activeTab === 'faq' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`} > Support & FAQ </button>
                                <button onClick={() => { setIsLoggedIn(false); setMobileMenuOpen(false); }} className="w-full text-left px-4 py-3 rounded-lg text-sm font-bold text-rose-400 border-t border-slate-800 mt-2" > Log Out </button>
                            </div>
                        )}
                    </header>

                    {/* MAIN WORKSPACE PANEL CONTENT VIEWPORT */}
                    <main className="flex-1 p-4 md:p-8 lg:p-10 max-w-7xl mx-auto w-full overflow-y-auto">
                        {activeTab === 'browse' && (
                            <div className="flex flex-col gap-6">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-5">
                                    <div>
                                        <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Academic Corpus Catalog</h1>
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        {categories.map((cat) => (
                                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${selectedCategory === cat ? 'bg-slate-900 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`} > {cat} </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="relative w-full bg-white rounded-2xl border border-slate-200 shadow-sm p-2 flex items-center gap-3">
                                    ...
                                    <input type="text" placeholder="Search titles, authors, specialized topics, or ISBN codes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none bg-transparent py-2.5" />
                                    {searchQuery && (
                                ...
                                {/* LITERATURE RESULTS GRID CONTAINER */}
                                {filteredBooks.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {filteredBooks.map((book) => (
                                            <div key={book.id} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col justify-between hover:shadow-md transition-shadow">
                                                <div>
                                                    <div className={`h-40 ${book.coverClass} p-5 flex flex-col justify-between relative`}>
                                                        <span className="bg-white/20 backdrop-blur-md text-white text-[11px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md self-start"> {book.category} </span>
                                                        <div className="flex items-center gap-1 bg-black/30 backdrop-blur-sm px-2 py-0.5 rounded text-amber-300 text-xs font-bold self-end absolute top-5 right-5"> <Star className="w-3 h-3 fill-current" /> {book.rating} </div>
                                                        <h3 className="text-white text-lg font-black tracking-tight leading-snug truncate-2-lines">{book.title}</h3>
                                                    </div>
                                                    <div className="p-5 flex flex-col gap-3">
                                                        <div className="flex flex-col">
                                                            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wide">Author</span>
                                                            <span className="text-sm font-bold text-slate-800">{book.author}</span>
                                                        </div>
                                                        <p className="text-xs text-slate-500 font-medium leading-relaxed line-clamp-3">{book.description}</p>
                                                        <div className="flex flex-col gap-1 border-t border-slate-100 pt-3 text-[11px] font-medium text-slate-400">
                                                            <div>ISBN: <span className="text-slate-600 font-bold">{book.isbn}</span></div>
                                                            <div>Published: <span className="text-slate-600 font-bold">{book.publishYear}</span></div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="p-5 pt-0 border-t border-slate-50 mt-auto flex items-center justify-between gap-3">
                                                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold ${book.status === 'available' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                                                        <span className={`w-1.5 h-1.5 rounded-full ${book.status === 'available' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                                                        {book.status === 'available' ? 'Available' : 'On Hold'}
                                                    </span>
                                                    <button onClick={() => openReservationWorkflow(book)} disabled={book.status !== 'available'} className={`px-4 py-2 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 ${book.status === 'available' ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md shadow-indigo-600/10' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`} >
                        ...
                        {activeTab === 'reservations' && (
                            <div className="flex flex-col gap-6">
                                <div className="border-b border-slate-200 pb-5">
                                    ...
                                    <p className="text-sm text-slate-500 font-medium mt-1">Track physical collection escrow queue status allocations.</p>
                                </div>
                                {reservations.length === 0 ? (
                                    <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center flex flex-col items-center justify-center max-w-xl mx-auto w-full shadow-sm mt-4">
                                        ...
                                        <p className="text-sm text-slate-500 font-medium mt-1 max-w-xs">You don't have any books currently reserved. Explore the catalog database to place an item on hold.</p>
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {reservations.map((res) => (
                                            <div key={res.reservationId} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden">
                                                <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-500/5 rounded-full blur-xl pointer-events-none"></div>
                                                <div className="flex flex-col gap-4">
                                                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                                        <span className="text-[11px] font-black font-mono bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-md">{res.reservationId}</span>
                                                        <span className="text-xs font-medium text-slate-400">Escrow Ready</span>
                                                    </div>
                                                    <div>
                                                        <h3 className="font-black text-slate-900 text-base tracking-tight leading-snug">{res.bookTitle}</h3>
                                                        <div className="grid grid-cols-2 gap-4 mt-4 text-xs font-medium text-slate-500">
                                                            <div>
                                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-0.5">Student ID</span>
                                                                <span className="text-slate-800 font-bold">{res.studentId}</span>
                                                            </div>
                                                            <div>
                                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-0.5">Pickup Window</span>
                                                                <span className="text-amber-600 font-bold">{res.pickupDeadline}</span>
                                                            </div>
                                                        </div>
                                                        <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-2 text-xs text-slate-600 font-medium">
                                                            <MapPin className="w-4 h-4 text-indigo-500 flex-shrink-0" />
                                                            <span className="truncate">Location: <strong className="text-slate-800 font-bold">{res.location}</strong></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="border-t border-slate-100 pt-4 mt-5 flex justify-end">
                                                    <button onClick={() => handleCancelReservation(res.reservationId, res.bookId)} className="px-4 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-xl transition-colors"> Release Hold </button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        {activeTab === 'faq' && (
                            <div className="flex flex-col gap-6">
                                <div className="border-b border-slate-200 pb-5">
                                    <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">System Documentation</h1>
                                    <p className="text-sm text-slate-500 font-medium mt-1">Frequently asked operational guidelines regarding academic resource distribution metrics.</p>
                                </div>
                                <div className="flex flex-col gap-3 max-w-3xl">
                                    {initialFAQs.map((faq) => (
                                        <div key={faq.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm transition-all">
                                            <button onClick={() => setExpandedFAQ(expandedFAQ === faq.id ? null : faq.id)} className="w-full px-6 py-4 text-left flex items-center justify-between font-bold text-slate-800 hover:text-indigo-600 text-sm transition-colors" >
                                                <span>{faq.question}</span>
                                                <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${expandedFAQ === faq.id ? 'transform rotate-180 text-indigo-500' : ''}`} />
                                            </button>
                                            {expandedFAQ === faq.id && (
                                                <div className="px-6 pb-5 pt-1 border-t border-slate-50 text-xs text-slate-500 font-medium leading-relaxed">
                                                    {faq.answer}
                                                </div>
                                            )}
                                        </div>
            ...
            {/* RESERVATION VERIFICATION WORKFLOW MODAL DIALOG */}
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6 pb-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                            <h3 className="font-black text-slate-900 text-lg tracking-tight">Confirm Hold Reservation</h3>
                            <button onClick={() => { setShowReservationModal(false); setSelectedBook(null); }} className="p-1 hover:bg-slate-200 rounded-lg text-slate-400 hover:text-slate-600 transition-colors">
                                <X className="w-4 h-4" />
                            </button>
                        </div>

                        <div className="p-6 flex flex-col gap-4 text-xs font-medium text-slate-600 leading-normal">
                            <p>You are requesting an escrow reservation hold placement for the following academic catalog module:</p>
                            <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl flex flex-col gap-1.5">
                                <div className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider">{selectedBook.category}</div>
                                <div className="font-black text-slate-900 text-sm tracking-tight leading-snug">{selectedBook.title}</div>
                                <div className="text-slate-400 mt-1">Author: <strong className="text-slate-700 font-bold">{selectedBook.author}</strong></div>
                            </div>
                            <div className="flex flex-col gap-2 bg-amber-50/40 border border-amber-100 p-4 rounded-xl text-amber-900">
                                <div className="font-bold flex items-center gap-1.5 text-[13px] tracking-tight text-amber-800">
                                    <AlertCircle className="w-4 h-4 flex-shrink-0" /> Important Escrow Agreement Clauses
                                </div>
                                <p className="text-[11px] font-medium leading-relaxed mt-1 text-amber-800/80">By placing a hold, this item gets locked to your account index. You must physically present your student profile matrix identification at the shelf location within the allocated collection frame.</p>
                            </div>
